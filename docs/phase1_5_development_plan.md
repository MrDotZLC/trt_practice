...
> 本文档定义的是**任务、顺序、产出与验收**。
>
> **本阶段没有独立的测试计划文件**（有意）：测试设计在 `docs/phase1_test_plan.md`（E1–E4），
> 任务/验收/执行结果在本文档 §0 与 §5。再写一份"Phase 1.5 测试计划"只会把 E1–E4 复述一遍，
> 形成两处来源（违反 `docs/PROGRESS.md` §2.13「唯一来源」）。**找 Phase 1.5 的测试口径就看这两处。**
