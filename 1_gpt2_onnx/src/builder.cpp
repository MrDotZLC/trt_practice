...
void buildEngine(const std::string &onnxPath, const std::string &enginePath,
    ...
    // GPT-2 有两个动态轴：batch 和 seq_len
    profile->setDimensions("input_ids", nvinfer1::OptProfileSelector::kMIN,
                           nvinfer1::Dims2{1, 1});  // ← Dims2，不是 Dims4
    profile->setDimensions("input_ids", nvinfer1::OptProfileSelector::kOPT,
                           nvinfer1::Dims2{1, 64});
    profile->setDimensions("input_ids", nvinfer1::OptProfileSelector::kMAX,
                           nvinfer1::Dims2{4, 512});
