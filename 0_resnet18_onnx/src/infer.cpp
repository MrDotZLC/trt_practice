...
// ── 构造函数
// ──────────────────────────────────────────────────────────────────
InferSession::InferSession(const std::string &enginePath, Logger &logger,
                           int maxBatch)
...
std::vector<float> InferSession::infer(const std::vector<float> &inputHost,
    ...
    m_context->setInputShape("input",
                             nvinfer1::Dims4{batchSize, k_C, k_H, k_W});
    ...
    // cudaMemcpyAsync(m_device_input, inputHost.data(), input_bytes,
    ...
    // cudaMemcpyAsync(outputHost.data(), m_device_output, output_bytes,
    // cudaMemcpyDeviceToHost, m_stream); D2H：改为拷贝到 Pinned Memory
    cudaMemcpyAsync(m_pinned_output, m_device_output, output_bytes,
                    cudaMemcpyDeviceToHost,
                    m_stream);  // ← dst 改为 m_pinned_output
    nvtxRangePop();             // NVTX 标注
...
void InferSession::benchmark(int batchSize, int nWarmup, int nRun,
    ...
    size_t input_bytes = batchSize * k_C * k_H * k_W * sizeof(float);
    cudaMemcpyAsync(m_device_input, input.data(), input_bytes,
                    cudaMemcpyHostToDevice, m_stream);
    cudaStreamSynchronize(m_stream);
    ...
    for (int i = 0; i < nRun; ++i) {
        ...
        // EventSynchronize：等待 evStop 完成（只等这一个 event，不等整个
        // stream）
    ...
    // ── 统计 ───────────────────────────────────────────────────────────────
    float mean =
        std::accumulate(latencies.begin(), latencies.end(), 0.f) / nRun;
