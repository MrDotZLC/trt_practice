...
TEST(PagedKVCacheTest, AppendCrossesBlockBoundaryAndAdvancesContextLens) {
    ...
    for (int32_t step = 0; step < 2; ++step) {
        const std::vector<float> step_key = {100.0f + step, 101.0f + step, 102.0f + step,
                                             103.0f + step, 104.0f + step, 105.0f + step,
                                             106.0f + step, 107.0f + step};
        DeviceBuffer d_key(step_key.size() * sizeof(float));
        DeviceBuffer d_value(step_key.size() * sizeof(float));
        ASSERT_TRUE(d_key.Allocate(step_key.size() * sizeof(float)));
        ASSERT_TRUE(d_value.Allocate(step_key.size() * sizeof(float)));
        CUDA_CHECK(cudaMemcpy(d_key.data(), step_key.data(), d_key.size(),
                              cudaMemcpyHostToDevice));
        CUDA_CHECK(cudaMemcpy(d_value.data(), step_key.data(), d_value.size(),
                              cudaMemcpyHostToDevice));
        ASSERT_EQ(cache.AppendDecodeStep({d_key.data()}, {d_value.data()}, nullptr),
                  cudaSuccess);
        CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (int32_t h = 0; h < kKvHeads; ++h) {
        for (int32_t d = 0; d < kHeadSize; ++d) {
            ...
            EXPECT_FLOAT_EQ(key_cache[ExpectedOffset(flat_table, 0, 3, h, d)],
                            100.0f + static_cast<float>(h * kHeadSize + d));
            EXPECT_FLOAT_EQ(key_cache[ExpectedOffset(flat_table, 0, 4, h, d)],
                            101.0f + static_cast<float>(h * kHeadSize + d));
        }
    }
}
