...
TEST(Gpt2OnnxTest, MatchesAcrossProfileShapes) {
    ...
    for (const auto& [batch, seq] : shapes) {
        ...
        // 逐位置 argmax（采样只看最后一行，但非最后一行错也说明中间层有问题）
        std::vector<int32_t> onnx_argmax;
        std::vector<int32_t> native_argmax;
        {
            // ArgmaxAll 目前按 kSeq 写死，这里就地做一次
            onnx_argmax.clear();
            native_argmax.clear();
            for (int32_t row = 0; row < batch * seq; ++row) {
                ...
                onnx_argmax.push_back(static_cast<int32_t>(
                    std::distance(a, std::max_element(a, a + kVocab))));
                native_argmax.push_back(static_cast<int32_t>(
                    std::distance(b, std::max_element(b, b + kVocab))));
            }
        }
        EXPECT_EQ(onnx_argmax, native_argmax) << "batch=" << batch << " seq=" << seq;
    }
}
